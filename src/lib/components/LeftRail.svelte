...
{#if collapsed}
	...
	>
		<IconButton label="Expand navigation" onclick={() => (collapsed = false)}>
			<PanelLeftOpen class="h-5 w-5" aria-hidden="true" />
		</IconButton>
		{#if warningCount > 0 && firstWarningId !== null}
			<IconButton
				label="{warningCount} integrity {warningCount === 1 ? 'warning' : 'warnings'} — review"
...
{:else}
	...
	>
		<div class="flex items-center gap-2">
			<h2 class="flex-1 text-xs font-semibold uppercase tracking-wide opacity-70">Views</h2>
			<IconButton label="Collapse navigation" onclick={() => (collapsed = true)}>
		...
		{#if warningCount > 0 && firstWarningId !== null}
			<button
				...
				onclick={reviewFirstWarning}
				aria-label="Review {warningCount} integrity {warningCount === 1 ? 'warning' : 'warnings'}"
			>
				<AlertTriangle class="h-4 w-4 shrink-0" aria-hidden="true" />
				<span>
					{warningCount}
					{warningCount === 1 ? 'integrity warning' : 'integrity warnings'}
				</span>
			</button>
		{/if}

		<h2 class="mt-2 text-xs font-semibold uppercase tracking-wide opacity-70">Filters</h2>
		<FilterBar />
	</aside>
{/if}
