...
describe('MemoryFsAdapter — constructor & seed', () => {
	...
	it('seeds initial files and creates parent directories', () => {
		const fs = new MemoryFsAdapter({
			files: {
				'.quill.md/config.json': '{"statuses":[]}',
				'.quill.md/issues/0001-first.md': '# first'
			}
		});
		const snap = fs.snapshot();
		expect(snap.files['.quill.md/config.json']).toBe('{"statuses":[]}');
		expect(snap.files['.quill.md/issues/0001-first.md']).toBe('# first');
		// The root + two ancestor directories should be registered.
		expect(snap.directories).toContain('.');
		expect(snap.directories).toContain('.quill.md');
		expect(snap.directories).toContain('.quill.md/issues');
	});
...
describe('MemoryFsAdapter — integration with loadIssues (service layer)', () => {
	 ...
	 * loadIssues() reads every *.md file under .quill.md/issues/, parses
...
`;
		...
		await fs.writeTextFile('.quill.md/issues/0042-fix-login-redirect.md', ersExample);
	...
	it('returns an empty list when the issues directory is missing', async () => {
		// Fresh filesystem — no .quill.md/issues/ at all.
		const fs = new MemoryFsAdapter();
		const issues = await loadIssues(fs);
